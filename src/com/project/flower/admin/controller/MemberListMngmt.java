...
@WebServlet("/Admin/MemberListMngmt")
public class MemberListMngmt extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//1. 전체 게시물 수량 구하기
		p.setTotalRecord(PagingDAO.memberTotCnt());
		p.setTotalPage();
		...
		List<ReviewVO> list = MemberDAO.memberList(p.getBegin(), p.getEnd());
		// 데이터를 화면에 표시(출력)
		...
		//List<MemberVO> list = MemberDAO.selectMemberAll();
